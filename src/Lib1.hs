...
myTranspose :: [[a]] -> [[a]]
myTranspose ([]:_) = []  -- Base case: empty sublists, stop transposing
myTranspose matrix
    | any null matrix = []  -- Check for empty sublists, stop transposing
    | otherwise = (map head matrix) : myTranspose (map tail matrix)
